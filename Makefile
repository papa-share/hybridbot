...
.PHONY: help install install-dev run run-watch test test-cov lint lint-fix format format-check typecheck deadcode check clean clean-db
...
help:
	...
	@echo "  make clean-db     - Supprimer chainlit.db"
...
clean-db:
	$(UV) run python -c "import pathlib; pathlib.Path('chainlit.db').unlink(missing_ok=True)"
