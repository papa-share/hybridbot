...
from chatbot.config import MIME_IMAGE_PREFIX, MIME_PDF, MIME_TEXT_PREFIX, config, logger
...
def validate_uploaded_files(files) -> tuple[list, list, list[str]]:
    ...
    for file in files:
        name = getattr(file, "name", "inconnu")
        ...
        if validate_file_type(file, [MIME_PDF, MIME_TEXT_PREFIX])[0]:
        ...
        errors.append(f"{name}: type non supporté ({mime})")
